...
class CorrectionStockViewModel: ViewModel() {

    val isDoneGetCategoryListLiveData: MutableLiveData<Boolean> = MutableLiveData<Boolean>()
    val isDoneCorrectionStock: MutableLiveData<Boolean> = MutableLiveData()

    var selectedCorrection: MutableLiveData<String> = MutableLiveData()
    ...
    fun getBahanFromCategoryList(auth: String, context: Context, request: CategoryIdListRequest) {
        RetrofitBuilder().getService().getBahanListByCategoryId(authHeader = auth, listIds = request).enqueue(object:
            Callback<BahanListResponseModel> {
    ...
    fun getRekapTodayList(auth: String, context: Context, request: RekapTodayRequest) {
        RetrofitBuilder().getService().getRekapToday(authHeader =  auth, rekapTodayRequest = request).enqueue(object:
            Callback<RekapTodayResponse> {
    ...
    fun updateRekapToday(auth: String, context: Context, request: UpdateRekapTodayRequest) {
        RetrofitBuilder().getService().updateRekapToday(authHeader = auth, updateRekapTodayRequest = request).enqueue(object:
            Callback<BaseResponse> {
            override fun onResponse(call: Call<BaseResponse>, response: Response<BaseResponse>) {
                val responseBody = response.body()
                if (responseBody?.status == true) {
                    isDoneCorrectionStock.value = true
                    Toast.makeText(context, "Sukses mengupdate stok", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(context, "Fail correction stock", Toast.LENGTH_SHORT).show()
                }
            }
